  ...
  methods: {
    ...
    verifyImage: function(e) {
      ...
      if (minuteGap > 5) { // 찍은 지 5분 이상이 지났으면
        // 이거 지금 데스크탑이라 오래된 이미지로 테스트... 나중에 코드 거꾸로 바꾸기
        ...
        // // ctx.drawImage(image, 0, 0, ctx.canvas.width, ctx.canvas.height, 0, 0, ctx.canvas.width, ctx.canvas.height);
        ...
        // image.onload = () => ctx.drawImage(image, 20, 20);
        ...
        if (result) {
          ...
          console.log(22222)
          // 인식이 제대로 안 됐으니 다시 찍어 달라고 모달 띄우기
